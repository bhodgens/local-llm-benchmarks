#!/usr/bin/env python3
"""Rerun tau2 for Qwopus3.6-35B-A3B-Coder-MTP Q4_K_M - the 2026-07-09 run
died at ~90 min (model server stopped answering; 10/15 sims infra-error).
Canonical protocol: LFM2.5 user sim on 3060, airline/15/seed42/conc2/30,
all 15 scored from results.json (errors=0). V100 exclusive (production
service stopped for the duration). Records corrected tau2 on the entry."""
...
def main():
    ...
    time.sleep(5)
    try:
        # agent: 35B-A3B MoE, full offload, 32K ctx for headroom, MTP n3 (model has bundled head)
        env = dict(os.environ)
        env["CUDA_VISIBLE_DEVICES"] = "0"
        logf = open(os.path.join(LOGS, "qwopus36_35b_tau2_rerun_server.log"), "w")
        ...
        log("agent up (V100:18096, MTP n3), identity OK")

        uenv = dict(os.environ)
        uenv["CUDA_VISIBLE_DEVICES"] = "1"
        ulogf = open(os.path.join(LOGS, "qwopus36_35b_tau2_rerun_usersim.log"), "w")
        ...
        cmd = ["uv", "run", "tau2", "run", "--domain", "airline",
               ...
               "--seed", "42", "--save-to", f"tau2_{safe}_rerun"]
        tenv = dict(os.environ)
        tenv["OPENAI_API_KEY"] = "none"
        t0 = time.time()
        with open(os.path.join(LOGS, "qwopus36_35b_tau2_rerun.log"), "w") as lf:
            subprocess.run(cmd, stdout=lf, stderr=subprocess.STDOUT, timeout=86400,
                           env=tenv, cwd=TAU2_DIR)
        elapsed = round(time.time() - t0, 1)

        # authoritative: per-sim rewards from results.json
        sdir = os.path.join(TAU2_DIR, "data", "simulations", f"tau2_{safe}_rerun", "results.json")
        ...
        log(f"tau2 rerun: all-15={final:.4f} (passes={passes}/15, scored={len(scored)}, "
        ...
        e.setdefault("failures", []).append({
            "benchmark": "tau2 (service-outage rerun)",
            "error": (f"2026-07-09 run invalid: model server stopped answering ~90 min in "
                      f"(10/15 sims infrastructure_error, 0s duration). Rerun 2026-09-07 "
                      f"canonical protocol: {final:.4f} (passes={passes}/15, infra_errors={infra}). "
                      f"Superseded 0.1333 (denominator audit) and 0.40 (original scored-only). "
                      f"Raw: {per_task}"),
            "timestamp": TS})
