#!/usr/bin/env python3
"""
Benchmark Qwopus3.6-27B-v2-MTP and Qwythos-27B-v1 on the V100 (32GB).
Runs LCB (75 problems) + tau2-bench (airline, 15 tasks) + tok/s probe.
Agent model runs on V100 (CUDA0, device 0).
User simulator for tau2: LFM on :8082 (3060, must be started separately).

Usage:
    /home/caimlas/bench-venv/bin/python scripts/run_qwopus_qwythos.py

    # Make sure user sim is running on 3060:
    # BINARY=/home/caimlas/git/llama.cpp/build/bin/llama-server
    # CUDA_VISIBLE_DEVICES=1 $BINARY --model /home/files/llms/LFM2.5-8B-A1B-Clean-RealWorld-v2-Q4_K_M.gguf \
...
# Both are 27B Qwen3.x dense models with MTP support.
# Qwythos-27B-Q4_K_M.gguf lives in the gguf/ subdir.
# Qwopus3.6-27B-v2-MTP-Q4_K_M.gguf lives in LLMS_DIR directly.
MODELS = [
    {
        "name": "Qwopus3.6-27B-v2-MTP Q4_K_M",
        "file": "Qwopus3.6-27B-v2-MTP-Q4_K_M.gguf",
        "lcb_model": "local/qwopus-27b-v2-mtp",
        "binary": BINARY,
    ...
    {
        "name": "Qwythos-27B-v1 Q4_K_M",
        "file": "gguf/Qwythos-27B-Q4_K_M.gguf",
        "lcb_model": "local/qwythos-27b",
        "binary": BINARY,
        "args": ["--gpu-layers", "99", "--ctx-size", "262144", "--ubatch-size", "512",
                 "--threads", "8", "--threads-batch", "8",
                 "--cache-type-k", "q8_0", "--cache-type-v", "q8_0"],
    ...
    },
    {
        ...
        "lcb_model": "local/muse-glimmer-30b",
        "binary": BINARY,
        "args": ["--gpu-layers", "99", "--ctx-size", "131072", "--ubatch-size", "512",
                 "--threads", "8", "--threads-batch", "8",
                 "--cache-type-k", "q8_0", "--cache-type-v", "q8_0"],
        "category": "30B Dense",
        "thinking": False,  # not a thinking model
        "gpu": "V100",
    },
]
...
def run_livecodebench(model_name, lcb_model):
    ...
    # LCB writes to a dir named by the DISPLAY name from lm_styles, not the model key
    ...
    # Parse results — check both possible output dir names (model key and display name)
    pass1 = None
    for dirname in [lcb_model.replace("/", "_"), model_name]:
        output_dir = os.path.join(LCB_DIR, "output", dirname)
